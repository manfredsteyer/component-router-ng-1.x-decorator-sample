...
function setFrom(Constructor, from) {
  if (Constructor)
    $.hide(Constructor, 'from', from || function(iterable) {
...
function Iterator(iterable) {
  if (!$iter.is(iterable) && 'next' in iterable)
...
$iter.create(FilterIterator, WRAPPER, function() {
  ...
  for (; ; ) {
    var step = iter.next();
    if (step.done || stepCall(iter, this[FN], step.value, this[ENTRIES]))
      return step;
  }
});
...
$iter.create(SkipIterator, WRAPPER, function() {
  for (; ; ) {
    ...
    if (step.done || !this[INDEX] || !this[INDEX]--)
      return step;
  }
});
